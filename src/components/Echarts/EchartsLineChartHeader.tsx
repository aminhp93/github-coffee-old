...
import {
  DATE_FORMAT,
  DEFAULT_END_TIME_FRAME,
  DEFAULT_START_TIME_FRAME,
  LIST_END_TIME_FRAME,
...
export default function EchartsLineChartHeader({
  handleChangeTime,
  handleRefresh,
}: IProps) {
  ...
  const [startTimeFrame, setStartTimeFrame] = useState(
    DEFAULT_START_TIME_FRAME.value
  );
  const [endTimeFrame, setEndTimeFrame] = useState(
    DEFAULT_END_TIME_FRAME.value
  );
  const [startDate, setStartDate] = useState<Date | null>(
    new Date(new Date().valueOf() - 1000 * 60 * 60 * 24 * 3)
  );
